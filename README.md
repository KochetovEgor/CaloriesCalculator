...
- Nginx (для frontend'а и прокcbрования запросов)

Сервис написан с применением чистой архитектуры, в нём выделены основные слои: domain, service, storage, controller, DTO. 

Реализован frontend приложения с помощью JavaScript, HTML и CSS. Для его создания и проксирования запросов с frontend'а на backend использован веб-сервер Nginx.

В сервисе реализовано логирование с помощью пакета "log/slog" и аутентификация пользователей с помощью JWT. Реализован паттерн Graceful Shutdown для корректного завершения работы сервиса.
...
Backend приложения реализован в виде HTTP API, работающем стандартно по адресу `http://localhost:8000`. Frontend приложения работает по стандартному адресу `http://localhost`. Документацию после запуска сервиса можно посмотреть по адресу `http://localhost:8080/#/`.
